...
// squid) pass the time. It automatically generates a random order in which to draw numbers
...
//
class Day20211204(private val input: List<String>) : DailyChallenge {

    companion object {
        fun create(): Day20211204 {
            val input = Utils.readFileAsListOfString("Day20211204.txt")
            return Day20211204(input)
        }
    }

    internal class Board(rawBoard: List<String>) {
        private val boardMatrix = createBoardMatrix(rawBoard)

        private fun createBoardMatrix(rawBoard: List<String>): Array<Array<Int>> {
            return rawBoard.toTypedArray().map { rawRow ->
                rawRow.split(" ").filterNot { "" == it }.map { it.toInt() }.toTypedArray()
            }.toTypedArray()
        }

        fun numberCalled(toCheck: Int): Boolean {
            boardMatrix.forEach { row ->
                val maybeIndex = row.indexOf(toCheck)
                if (maybeIndex >= 0) {
                    row[maybeIndex] = -1
                    return true
                }
        ...
        fun isWinningBoard(): Boolean {
            for (row in 0..4) {
                if (boardMatrix[row].sum() == -5)
                    return true;
            }
            for (col in 0 .. 4) {
                if (boardMatrix.map { it[col] }.sum() == -5)
                    return true
            }
            return false
        }

        fun getUnmarkedSums(): Int {
            return boardMatrix.sumOf { row ->
                row.filter { it >= 0 }.sum()
            }
        }
    ...
    private val numbers = input[0].split(",").map(String::toInt)

    private val boardCount = (input.size - 1) / 6
    ...
    private fun findWinningBoardScore(someBoards: List<Board>): Int? {
        numbers.forEach { number ->
            val maybeWinningBoard: Board? = someBoards
                .filter { board -> board.numberCalled(number) }
                .find { board -> board.isWinningBoard() }
            if (maybeWinningBoard != null) {
                return (number * maybeWinningBoard.getUnmarkedSums())
            }
        }
        return null
    ...
    override fun first(): String {
        return findWinningBoardScore(boards)?.toString()?:"Foo"
    }

    override fun second(): String {
        numbers.forEach { number ->
            boards.forEach { it.numberCalled(number) }
            val losingBoards = boards.filterNot { board -> board.isWinningBoard() }
            if (losingBoards.size == 1) {
                return findWinningBoardScore(losingBoards)?.toString()?:"Bar"
            }
        }
        return "Baz"
    }

    override fun getLabel() = "2021/12/04 - Giant Squid"
}
