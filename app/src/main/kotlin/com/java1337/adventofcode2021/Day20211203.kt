...
// using a similar process that involves filtering out values until only one remains. Before
...
// (Be sure to represent your answer in decimal, not binary.)
class Day20211203(private val diagnosticReport: List<String>) : DailyChallenge {

    companion object {
        fun create(): Day20211203 {
            val input = Utils.readFileAsListOfString("Day20211203.txt")
            return Day20211203(input)
        }
    }

    private fun convert(strings: List<String>, column: Int): Char  {
        val half : Int = strings.size / 2
        val ones = strings.count {
            it[column] == '1'
        }
        return if (ones >= half) '1' else '0'
    }


    private fun matchBits(common: (List<String>) -> List<Char>, readings: List<String>, index:Int = 0): String {
        return if (readings.size == 1) {
            readings[0]
        } else {
            val bit = common(readings)[index]
            matchBits(common, readings.filter { row -> row[index] == bit }, index + 1)
        }
    }

    val mostCommon : (List<String>) -> List<Char> = { readings ->
        readings[0].mapIndexed { column, _ -> convert(readings, column) }
    }

    val leastCommon : (List<String>) -> List<Char> = { readings ->
        mostCommon(readings).map { if (it == '1') '0' else '1' }
    }

    private fun binaryArrayToInteger(input : List<Char>): Int {
        return Integer.parseInt(input.joinToString(""), 2)
    }

    override fun first(): String {
        val binaryGamma = mostCommon(diagnosticReport)
        val binaryEpsilon = leastCommon(diagnosticReport)
        val gamma = binaryArrayToInteger(binaryGamma)
        val epsilon = binaryArrayToInteger(binaryEpsilon)
        return (gamma * epsilon).toString()
    }

    override fun second(): String {
        val binaryOxygenGeneratorRating: List<Char> = matchBits(mostCommon, diagnosticReport).toCharArray().asList()
        val binaryCo2ScrubberRating: List<Char> = matchBits(leastCommon, diagnosticReport).toCharArray().asList()

        println("binaryOxygenGeneratorRating = $binaryOxygenGeneratorRating, binaryCo2ScrubberRating = $binaryCo2ScrubberRating")

        val oxygenGeneratorRating = binaryArrayToInteger(binaryOxygenGeneratorRating)
        val co2ScrubberRating = binaryArrayToInteger(binaryCo2ScrubberRating)
        return (oxygenGeneratorRating * co2ScrubberRating).toString()
    }

    override fun getLabel() = "2021/12/03 - Binary Diagnostic"
}
