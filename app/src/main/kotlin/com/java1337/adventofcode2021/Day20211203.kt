...
class Day20211203(private val diagnosticReport: List<String>) : DailyChallenge {
    ...
    override fun first(): String {
        var (gamma, epsilon) = Pair("", "")
        for (bit in 0.until(diagnosticReport[0].length)) {
            val counts = diagnosticReport.groupingBy { s -> s.at(bit) }.eachCount()

            gamma += if ((counts['0'] ?: 0) > (counts['1'] ?: 0)) '0' else '1'
            epsilon += if ((counts['0'] ?: 0) > (counts['1'] ?: 0)) '1' else '0'
        }

        return (gamma.binary() * epsilon.binary()).toString()
    }

    override fun second(): String {
        val oxygenInputs: MutableCollection<String> = diagnosticReport.toMutableList()
        val co2Inputs: MutableCollection<String> = diagnosticReport.toMutableList()
        var (oxygen, co2) = Pair("", "")
        ...
        return (oxygen.binary() * co2.binary()).toString()
    }

    private fun String.binary() = Integer.parseInt(this, 2)

    private fun <T> Collection<T>.head() = if (this.isNotEmpty()) this.first() else null

    private fun <T> MutableCollection<T>.reset(coll: Collection<T>) : MutableCollection<T> {
        this.clear()
        this.addAll(coll)
        return this
    }

    private fun String.at(pos: Int) = this[pos % length]
