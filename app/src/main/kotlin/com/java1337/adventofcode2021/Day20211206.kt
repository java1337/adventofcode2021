...
//     * After another day, the first lanternfish would have an internal timer of 5, and the
//       second lanternfish would have an internal timer of 7.
//
// A lanternfish that creates a new fish resets its timer to 6, not 7 (because 0 is included as
// a valid timer value). The new lanternfish starts with an internal timer of 8 and does not
...
class Day20211206(private val input: List<String>) : DailyChallenge {

    companion object {
        fun create(): Day20211206 {
            val input = Utils.readFileAsListOfString("Day20211206.txt")
            return Day20211206(input)
        }
    }
    ...
    internal class LanternfishSchool(private val rawSchoolDayZero: String) {
        ...
        init {
            rawSchoolDayZero
                .split(",")
                .forEach { fishDay ->
                    totalFishAtDay[fishDay.toInt()]++
                }
        }

        fun anotherDay() {
            val originalZeroAmount = totalFishAtDay[0]
            for (i in 0 .. 8) {
                totalFishAtDay[i] = when (i) {
                    8 -> originalZeroAmount
                    6 -> originalZeroAmount + totalFishAtDay[i+1]
                    else -> totalFishAtDay[i+1]
                }
            }
        }

        fun total(): Long = totalFishAtDay.sum()
    }
    ...
    private fun calclulate(days: Int): String {
        ...
        return school.total().toString()
    }

    override fun first(): String {
        return calclulate(80)
    }

    override fun second(): String {
        return calclulate(256)

    }

    override fun getLabel() = "2021/12/06 - Lanternfish"
}
