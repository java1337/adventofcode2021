...
// wires a through g, but those wires are connected to segments randomly. Worse, the wire/segment
// connections are mixed up separately for each four-digit display! (All of the digits
...
// ways the submarine tries to render a digit using the current wire/segment connections. Because
// 7 is the only digit that uses three segments, dab in the above example means that to render a 7,
...
// tell which combinations of signals correspond to those digits. Counting only digits in the
// output values (the part after | on each line), in the above example, there are 26 instances
...
//
// Adding all the output values in this larger example produces 61229.
//
// For each entry, determine all the wire/segment connections and decode the four-digit
// output values. What do you get if you add up all the output values?
//


class Day20211208(input: List<String>) : DailyChallenge {

    companion object {
        fun create(): Day20211208 {
            val input = Utils.readFileAsListOfString("Day20211208.txt")
            return Day20211208(input)
        }
    }

    private class Row(input: String) {
        val prefix: List<Set<Char>>
        val suffix: List<Set<Char>>
        val segmentsPerNumber = Array(10) { setOf<Char>() }
        ...
        init {
            ...
            prefix = parts.subList(0, 10)
            suffix = parts.subList(10, parts.size)

            val segmentsPerSize = prefix.groupBy { it.size }
            segmentsPerNumber[1] = segmentsPerSize[2]!!.first()
            segmentsPerNumber[4] = segmentsPerSize[4]!!.first()
            segmentsPerNumber[7] = segmentsPerSize[3]!!.first()
            segmentsPerNumber[8] = segmentsPerSize[7]!!.first()
            segmentsPerNumber[6] = segmentsPerSize[6]!!.first { segmentsPerNumber[8] == segmentsPerNumber[1] + it }
            segmentsPerNumber[5] = segmentsPerSize[5]!!.first { segmentsPerNumber[8] != segmentsPerNumber[6] + it }
            segmentsPerNumber[2] = segmentsPerSize[5]!!.first { segmentsPerNumber[8] == segmentsPerNumber[5] + it }
            segmentsPerNumber[9] = segmentsPerSize[6]!!.first { segmentsPerNumber[8] != segmentsPerNumber[4] + it }
            segmentsPerNumber[0] = segmentsPerSize[6]!!.first { segmentsPerNumber[8] == segmentsPerNumber[5] + it }
            segmentsPerNumber[3] = segmentsPerSize[5]!!.first { it !in segmentsPerNumber }
        }

        fun countEasySuffixes() : Int {
            return suffix.count {
        ...
        fun parseSuffix() : Int {
            return suffix.map { segmentsPerNumber.indexOf(it) }.joinToString("").toInt()
        }
    }

    private val rows = input.map { Row(it) }

    override fun first(): String {
        return rows.sumOf { it.countEasySuffixes() }.toString()
    }

    override fun second(): String {
        return rows.sumOf { it.parseSuffix() }.toString()
    }

    override fun getLabel() = "2021/12/08 - Seven Segment Search"
}
