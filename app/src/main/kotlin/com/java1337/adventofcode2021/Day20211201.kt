...
// the previous sum. So, compare A with B, then compare B with C, then C with D, and so on. Stop when there aren't
...
class Day20211201(private val depths: List<Int>): DailyChallenge {

    companion object {
        fun create(): Day20211201 {
            val input = Utils.readFileAsListOfString("Day20211201.txt").map(String::toInt)
            return Day20211201(input)
        }
    }

    private fun calculateIncrements(inputs: List<Int>) : Int {
    ...
    private fun calculateRunningDepthThree(input: List<Int>) : List<Int> {
        var previous1: Int = -1
        var previous2: Int = -1
        return input.map { current ->
            val sum = current + previous1 + previous2
            previous2 = previous1
            previous1 = current
            sum
        }.subList(2, depths.size)
    }

    override fun first() = calculateIncrements(depths).toString()
    override fun second() = calculateIncrements(calculateRunningDepthThree(depths)).toString()
    override fun getLabel() = "2021/12/01 - Submarine Depths"
}