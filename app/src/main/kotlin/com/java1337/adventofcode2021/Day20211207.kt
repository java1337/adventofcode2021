...
// otherwise) zooms in to rescue you! They seem to be preparing to blast a hole in the ocean
// floor; sensors indicate a massive underground cave system just beyond where they're aiming!
//
// The crab submarines all need to be aligned before they'll have enough power to blast a
// large enough hole for your submarine to get through. However, it doesn't look like they'll
// be aligned before the whale catches you! Maybe you can help?
//
...
class Day20211207(input: List<String>) : DailyChallenge {

    companion object {
        fun create(): Day20211207 {
            val input = Utils.readFileAsListOfString("Day20211207.txt")
            return Day20211207(input)
        }
    }

    private val positions = input[0].split(",").map{ it.toInt() }.sorted()

    private fun calculate(costToMoveTo: (Int, Int) -> Int): Int {
        return (positions.first()..positions.last()).minOf { position ->
            positions.sumOf { costToMoveTo(position, it) }
        }
    }

    override fun first(): String {
        fun costToMoveTo(oldPosition: Int, newPosition: Int): Int {
            return (oldPosition - newPosition).absoluteValue
        }
        return calculate(::costToMoveTo).toString()
    }

    override fun second(): String {
        fun getArithmeticSequenceSum(i : Int): Int {
            return i * (i + 1) / 2
        }

        fun costToMoveTo(oldPosition: Int, newPosition: Int): Int {
            return getArithmeticSequenceSum((oldPosition - newPosition).absoluteValue)
        }
        return calculate(::costToMoveTo).toString()
    }

    override fun getLabel() = "2021/12/07 - The Treachery of Whales"
}
